...
<head>
    ...
    <script>
                ...
                pop = layer.open({
                    ...
                    cancel: function(index, layero){
                        if(confirm('确定要关闭么')){ //只有当点击confirm框的确定时，该层才会关闭
                            if($("#lastImg").val()!=null&&$("#lastImg").val()!=""){
                                //关闭的时候删除上传的文件
                                $.ajax({
                                    url:"/upload/cancelUploadFile",
                                    type:"post",
                                    data:{lastImg:$("#lastImg").val()}
                                })
                            }
                            /*$("#lastImg").val("")
                            $("#pic").val("")*/
                            layer.close(index)
                            $("#addAlbumForm").resetForm()
                            $("#lastImg").val("")
                            $("#pic").val("")
            ...
            $("[modify]").click(function () {
                ...
                $.ajax({
                    ...
                    success: function (jsonObj) {
                        console.log(jsonObj)
                        $("#uaid").val(jsonObj.aid);
                        $("#uaname").val(jsonObj.aname);
                        $("#ucompany").val(jsonObj.company);
                        $("#uaddPdate").val(jsonObj.pdate);
                        $("#ulang1").val(jsonObj.lang);
                        $("#ualbuma").attr("href","${filePath}"+jsonObj.pic);
                        $("#ualbumImg").attr("src","${filePath}"+jsonObj.pic);
                        $("#mypic").val("${filePath}"+jsonObj.pic);
                        $("#ulastImg").val("${filePath}"+jsonObj.pic);
                        $("#upic").val(jsonObj.pic);
        ...
        function submitFile1() {
            $('#ulocation').val($('#ui-file').val());  //图片的全路径
            $('#updateAlbumForm').ajaxSubmit({
                ...
                dataType:"json",
                success:function (json) {
                    $("#ualbuma").attr("href",json.realPath);
                    $("#ualbumImg").attr("src",json.realPath);
                    $("#ulastImg").val(json.realPath);
                    $("#upic").val(json.relativePath);
                }
                                    ...
                                    <div class="control-group">
                                        ...
                                        <div class="controls form-group">
                                            <div class="input-group"> <span class="input-group-addon"><i class="icon-reorder"></i></span>
                                                <input readonly id="searchPdate" value="<f:formatDate value='${album.pdate}' pattern='yyyy-MM-dd'/>" type="text" placeholder="如：1970-01-01" name="pdate"  class="form-control"/>
                                            </div>
                                        </div>
...
<div id="albumPop1" style="margin-right: 50px;margin-top: 50px; display: none">
    <form id="updateAlbumForm" class="layui-form" method="post" action="/album/updateAblum" lay-filter="example">
        <div class="layui-form-item" >
            <label class="layui-form-label" style="width:100px">专辑名字</label>
            <div class="layui-input-block">
                <input  id="uaname" type="text" name="aname" style="color: black; border-color: lightgray;background-color: white"  lay-verify="aname" autocomplete="off" placeholder="请输入专辑名" class="layui-input">
            </div>
        </div>
        ...
        <div class="layui-form-item" >
            <label class="layui-form-label" style="width:100px">发行公司</label>
            <div class="layui-input-block">
                <input id="ucompany" type="text" name="company" style="color: black; border-color: lightgray;background-color: white"  lay-verify="company" autocomplete="off" placeholder="请输入公司名" class="layui-input">
            </div>
        </div>
        ...
        <div class="layui-form-item" >
            <label class="layui-form-label" style="width:100px">发行时间</label>
            <div class="layui-input-block">
                <input  type="text" id="uaddPdate"  name="pdate" style="color: black; border-color: lightgray;background-color: white"  lay-verify="pdate" autocomplete="off" placeholder="请输入发行时间" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            <label  class="layui-form-label" style="width:100px">图片</label>
            <div class="controls form-group">
                <div class="col-sm-4 col-md-2">
                    <div class="image-row">
                        <div class="image-set"> <a class="example-image-link" id="ualbuma" href="../../img/gallery-photo/image-3.jpg" data-lightbox="example-set" title="Click on the right side of the image to move forward.">
                            <img class="example-image" id="ualbumImg" src="../../img/gallery-photo/thumb-3.jpg" alt="Plants: image 1 0f 4 thumb" width="150" height="150" /></a> </div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="layui-form-item">
            <label for="ui-file" class="layui-form-label" style="width:100px">选择文件</label>
            <!--<div class="col-sm-4 control-label">选择文件</div>-->
            <div id="uexamples" class="section examples-section">
                <div class="col-sm-6">
                    <div class="input-group">
                        <input id='ulocation' class="form-control" onclick="$('#ui-file').click();">
                        <label class="input-group-btn">
                            <input  type="button" id="ui-check" value="选择封面" class="btn btn-primary" onclick="$('#ui-file').click();">
                        </label>
                    </div>
                </div>
                <%--在选择一个文件的时候触发的事件onchange--%>
                <input type="hidden" id="mypic">
                <input type="hidden" id="uaid" name="aid">
                <input type="hidden" id="upic" name="pic" lay-verify="pic">
                <input type="hidden" id="ulastImg" name="lastImg">
                <input type="file" name="picFile" id='ui-file'  accept=".jpg, .png" onchange="submitFile1()" style="border-color: lightgray;background-color: lightgray;display: none">
            </div>
        </div>
        ...
        <div class="layui-form-item" >
            <label for="lang1" class="layui-form-label " style="width:100px">语种</label>
            <div class="layui-input-block">
                <input id="ulang1"  type="text" name="lang" style="color: black; border-color: lightgray;background-color: white"  lay-verify="lang" autocomplete="off" placeholder="请输入语种" class="layui-input">
            </div>
        </div>
